...
- `POST /api/stt` - 语音识别
- `GET /api/kg/category/list` - 知识图谱分类列表
- `GET /api/kg/category/list/all` - 知识图谱全部分类
- `GET /api/kg/document/list` - 知识图谱文档列表
- `GET /api/kg/document/{id}` - 知识图谱文档详情
- `GET /api/kg/graph/{categoryId}` - 获取图谱数据
- `GET /api/kg/graph/vertex/{id}` - 获取子图数据
...
> 详见 [二十、知识图谱模块](#二十知识图谱模块) 中的管理后台接口（10-14）。

---

## 二十、知识图谱模块

> 基于 AI 自动提取的知识图谱系统，支持文档上传、自动分类、知识点提取、关系分析和可视化展示。
> 前台公开接口无需登录，管理后台接口需要管理员角色（roleType=3）。
> AI 处理流水线：上传文档 → 自动分类（MiMo）→ 提取知识点（MiMo）→ 分析关系（DeepSeek）→ 构建邻接缓存。

### 1. 获取分类列表

- **URL**: `GET /api/kg/category/list`
- **需要登录**: 否

**说明:** 返回所有已启用的知识图谱分类，用于前台图谱导航。
...
- **URL**: `GET /api/kg/category/list/all`
- **需要登录**: 否

**说明:** 返回所有分类（包括禁用的），用于管理后台筛选。

**成功响应:** 同上，包含 `status` 为 `disabled` 的分类。

---

### 3. 获取文档列表

- **URL**: `GET /api/kg/document/list`
- **需要登录**: 否
...
| 字段 | 类型 | 说明 |
|---|---|---|
| parseStatus | String | 处理状态：pending-待处理 processing-处理中 completed-已完成 failed-失败 |
| currentStep | String | 当前处理步骤：parse/extract/analyze/adjacency（处理中时有值） |
| vertexCount | Integer | 提取的知识点数量 |
| edgeCount | Integer | 提取的关系数量 |

---

### 4. 获取文档详情

- **URL**: `GET /api/kg/document/{id}`
- **需要登录**: 否
- **路径参数:**
...
- **URL**: `GET /api/kg/graph/{categoryId}`
- **需要登录**: 否
- **路径参数:**
...
**说明:** 返回指定分类下所有知识点（节点）和关系（边），用于 ECharts 力导向图渲染。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| vertexType | String | 知识点类型：concept-概念 technology-技术 api-API theory-理论 example-示例 keyword-关键词 |
| edgeLabel | String | 关系标签（如：包含、依赖、继承、实现） |
| weight | Double | 关系权重（0-1），越大连线越粗 |

---

### 6. 获取子图数据

- **URL**: `GET /api/kg/graph/vertex/{id}`
- **需要登录**: 否
- **路径参数:**
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|
| depth | Integer | 否 | 1 | 展开深度（1=一跳邻居，2=二跳） |

**说明:** 获取指定知识点及其 N 跳邻居的子图数据。
...
- **URL**: `GET /api/kg/graph/neighbor/{vertexId}`
- **需要登录**: 否
- **路径参数:**
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| keyword | String | 是 | 搜索关键词（匹配知识点名称和描述） |
| categoryId | Long | 否 | 分类ID筛选（不传则搜全部） |
...
- **URL**: `GET /api/kg/graph/stats/{categoryId}`
- **需要登录**: 否
- **路径参数:**
...
- **URL**: `POST /api/admin/kg/upload`
- **Content-Type**: `multipart/form-data`
- **需要登录**: 是（管理员角色）

**说明:** 上传 Markdown/TXT 文档，AI 自动提取知识点和关系。上传后异步处理，通过 SSE 推送进度。

**请求参数:**

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| file | File | 是 | 文档文件（支持 .md / .txt / .markdown） |
| categoryName | String | 否 | 分类名称（如"Java"，不填则 AI 自动分类） |
...
- **URL**: `GET /api/admin/kg/categories`
- **需要登录**: 是（管理员角色）

**说明:** 返回所有分类（包括禁用的），用于管理后台文档上传时的分类筛选。
...
- **URL**: `POST /api/admin/kg/retry/{documentId}`
- **需要登录**: 是（管理员角色）
- **路径参数:**

| 参数 | 类型 | 说明 |
|---|---|---|
| documentId | Long | 文档ID（状态需为 failed） |

**说明:** 重新触发 AI 处理流水线。前端需重新订阅 SSE 获取进度。
...
- **URL**: `DELETE /api/admin/kg/document/{id}`
- **需要登录**: 是（管理员角色）
- **路径参数:**
...
- **URL**: `GET /api/admin/kg/subscribe?token={jwtToken}&documentId={id}`
- **需要登录**: 是（通过 URL 参数携带 Token）
- **Content-Type**: `text/event-stream`

**说明:** 建立 SSE 长连接，实时接收文档 AI 处理进度。管理员上传文档后调用。

**查询参数:**

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| token | String | 是 | JWT Token（管理员） |
...
|-------|------|----------|
| connected | 连接成功心跳 | `"ok"` |
| kg_progress | 处理进度推送 | JSON 对象（见下表） |

**kg_progress 事件 data 字段说明:**

| 字段 | 类型 | 说明 |
|---|---|---|
| documentId | Long | 文档ID |
| step | String | 当前步骤：parse/extract/analyze/adjacency（完成/失败时为空） |
| status | String | 状态：processing/completed/failed |
| progress | Integer | 进度百分比（0-100） |
| vertexCount | Integer | 已提取的知识点数量 |
| edgeCount | Integer | 已提取的关系数量 |
| errorMessage | String | 错误信息（status=failed 时有值） |

**进度推进示例:**

```
connected: "ok"
kg_progress: {"step":"parse","status":"processing","progress":20}
kg_progress: {"step":"extract","status":"processing","progress":60,"vertexCount":37}
kg_progress: {"step":"analyze","status":"processing","progress":85,"vertexCount":37,"edgeCount":42}
kg_progress: {"step":"adjacency","status":"processing","progress":100,"vertexCount":37,"edgeCount":42}
kg_progress: {"step":"","status":"completed","progress":100,"vertexCount":37,"edgeCount":42}
```

**失败示例:**

```
kg_progress: {"step":"","status":"failed","progress":0,"errorMessage":"AI 提取知识点失败"}
```
...
|---|---|---|
| id | Long | 主键ID |
| name | String | 分类名称（如：Java、前端、数据库） |
| slug | String | URL 友好标识（如：java、frontend） |
| description | String | 分类描述 |
...
| fileUrl | String | MinIO 文件地址 |
| rawContent | String | 文档原始文本内容 |
| categoryId | Long | 所属分类ID（可为空，AI自动分类） |
| parseStatus | String | 处理状态：pending/processing/completed/failed |
| currentStep | String | 当前处理步骤：parse/extract/analyze/adjacency |
| vertexCount | Integer | 提取的知识点数量 |
| edgeCount | Integer | 提取的关系数量 |
...
| id | Long | 主键ID |
| uuid | String | 全局唯一标识 |
| documentId | Long | 来源文档ID |
| categoryId | Long | 所属分类ID |
| name | String | 知识点名称 |
| description | String | 知识点描述 |
| vertexType | String | 类型：concept/technology/api/theory/example/keyword |
...
| fromId | Long | 起始知识点ID |
| toId | Long | 目标知识点ID |
| edgeLabel | String | 关系标签（如：包含、依赖、继承） |
| weight | Double | 关系权重（0-1） |
| metadata | String | 扩展元数据 JSON |
| createdAt | LocalDateTime | 创建时间 |
...
| id | Long | 主键ID |
| vertexId | Long | 知识点ID |
| neighborId | Long | 邻居知识点ID |
| directLabel | String | 关系标签 |
| direction | String | 方向：outgoing-出边 incoming-入边 |
| weight | Double | 关系权重 |
| createdAt | LocalDateTime | 创建时间 |

**说明:** 邻接缓存表用于快速查询一跳邻居，由文档处理流水线的最后一步自动构建。

---

### KgProcessingTask（处理任务记录）

| 字段 | 类型 | 说明 |
|---|---|---|
| id | Long | 主键ID |
| documentId | Long | 关联文档ID |
| step | String | 处理步骤：parse/extract_vertices/analyze_edges/build_adjacency |
